...
In a standalone clone, output is `releases/ConsistentVaulting.zip`. Within the shared mod-development workspace, the existing packager selects that workspace's base `releases/` directory. No extra release directories are needed. Intermediate output stays in ignored `build/`; building does not install, launch or modify the game.
